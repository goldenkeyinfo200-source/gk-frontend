...
import { ArrowLeftRight, Send, CheckCircle, XCircle, Clock, Users, ChevronDown } from 'lucide-react'
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex items-center justify-between">
        ...
        <Btn onClick={() => setSendOpen(true)}><Send size={16} /> Lid yuborish</Btn>
      </div>
      ...
      <div className="flex gap-1 bg-white border border-cherry-100 rounded-2xl p-1">
        {[
          ...
          <button key={t.key} onClick={() => setType(t.key)}
            className={clsx('flex-1 py-2 rounded-xl text-xs sm:text-sm font-medium transition-all',
              type === t.key ? 'bg-cherry-700 text-white' : 'text-gray-500 hover:text-cherry-700')}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <SendLeadModal open={sendOpen} onClose={() => setSendOpen(false)} onSent={() => { setSendOpen(false); load() }} />
    </div>
  )
}
...
function LeadCard({ lead: l, type, onRespond }) {
  const isIncoming = type === 'incoming' || (type === '' && l.receiver_id)
    ...
    <div className={clsx('card p-4', l.status === 'pending' && 'border-amber-200')}>
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-full bg-cherry-100 flex items-center justify-center text-xs font-semibold text-cherry-700 flex-shrink-0">
          {(isIncoming ? l.sender_name : l.receiver_name)?.[0] || 'A'}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="text-sm font-semibold text-gray-900">
              {isIncoming ? l.sender_name : l.receiver_name}
            </span>
            <span className="text-xs text-gray-400">{l.display_id}</span>
            <Badge color={STATUS_COLOR[l.status]}>{STATUS_UZ[l.status]}</Badge>
          </div>
          <p className="text-xs text-gray-600">
            {l.client_name} · {TYPE_UZ[l.property_type]} · {NEED_UZ[l.need_type]}
          </p>
          <p className="text-xs text-gray-500 mt-0.5">
            Byudjet: {fmt(l.budget_min)} — {fmt(l.budget_max)}
          </p>
          {l.client_phone && (
            <a href={`tel:${l.client_phone}`} className="text-xs text-cherry-600 hover:underline flex items-center gap-1 mt-1">
              📞 {l.client_phone}
            </a>
          )}
          {l.notes && (
      ...
      {isIncoming && l.status === 'pending' && (
        <div className="flex gap-2 mt-3 pt-3 border-t border-cherry-50">
          <Btn variant="primary" size="sm" className="flex-1" onClick={() => onRespond(l.id, 'accept')}>
            <CheckCircle size={13} /> Qabul qilish
          </Btn>
          <Btn variant="outline" size="sm" className="flex-1" onClick={() => onRespond(l.id, 'reject')}>
...
function SendLeadModal({ open, onClose, onSent }) {
  ...
  const [form, setForm]         = useState({ client_id: '', receiver_id: '', notes: '' })
  const [loading, setLoading]   = useState(false)
  const set = (k, v) => setForm(p => ({ ...p, [k]: v }))

  useEffect(() => {
    if (open) clientsApi.list({ status: 'active' }).then(r => setClients(r.data))
  }, [open])

  const submit = async (e) => {
    e.preventDefault()
    if (!form.client_id || !form.receiver_id) return toast.error("Barcha maydonlarni to'ldiring")
    setLoading(true)
    try {
      await leadsApi.send(form)
      toast.success('Lid yuborildi! Telegram orqali xabar ketdi.')
      onSent()
    } catch (err) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Lid yuborish" size="sm">
      <form onSubmit={submit} className="space-y-4">
        <Select label="Mijoz tanlang" value={form.client_id} onChange={e => set('client_id', e.target.value)}>
        ...
        <div>
          <label className="text-xs font-medium text-gray-600 mb-1.5 block">Qabul qiluvchi agent ID</label>
          <input
          ...
          />
          <p className="text-xs text-gray-400 mt-1">Agent profili sahifasidan ID ni olish mumkin</p>
        </div>
        <Textarea label="Izoh (ixtiyoriy)" value={form.notes} onChange={e => set('notes', e.target.value)} placeholder="Qo'shimcha ma'lumot..." />
        <div className="flex gap-2">
          <Btn type="button" variant="outline" onClick={onClose} className="flex-1">Bekor</Btn>
          <Btn type="submit" loading={loading} className="flex-1"><Send size={14} /> Yuborish</Btn>
        </div>
      </form>
    </Modal>
  )
